// ✅ UPDATED COMPONENT: PurchaseCalculator.jsx for claculation on just car price, down payment, interest rate, and loan term
...
const PurchaseCalculator = ({ carPrice, carModel, carName, onConfirm }) => {
  ...
  const handleCalculate = () => {
    ...
    if (isNaN(price) || isNaN(down) || isNaN(term) || isNaN(annualRate)) {
    ...
    const monthly = (loanAmount * monthlyRate * Math.pow(1 + monthlyRate, term)) /
  ...
  const handleConfirm = () => {
    ...
    setError('');
    onConfirm && onConfirm({ carName, carModel, downPayment, months, interestRate, monthlyPayment });
  };
  ...
  return (
    <div className="mt-4">
      <Form>
        ...
        <Button variant="success" className="me-2" onClick={handleCalculate}>Calculate Monthly Payment</Button>
        <Button variant="primary" onClick={handleConfirm}>Confirm Purchase</Button>
      </Form>
      {monthlyPayment !== null && (
        <Alert variant="success" className="mt-3">
          Monthly Payment: <strong>{monthlyPayment.toFixed(2).toLocaleString()} THB</strong>
        </Alert>
      )}
